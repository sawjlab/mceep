...
c                                           -JJL 6/6/01
     ...
     +              dy(2),dd(2),npts(2)
c
c      COMMON /DATDIR_C/ DAT_DIR
c      COMMON /DATDIR_I/ N_DAT_DIR
C
      CHARACTER*100  DAT_DIR
      CHARACTER*200  FILELOC
      INTEGER        N_DAT_DIR,IARM,NLOC
      LOGICAL        COLLIMATOR
c
      integer ndum
      character*24 daet
c
      call getenv('mceep_dat', dat_dir)
      IF(IARM.EQ.1 .AND. COLLIMATOR) THEN
         FILELOC = DAT_DIR(:lnblnk(dat_dir))//
     >'/r-function/leftarm/collimator'
      ELSEIF(IARM.EQ.1 .AND. .NOT.COLLIMATOR) THEN
         FILELOC = DAT_DIR(:lnblnk(dat_dir))//
     >'/r-function/leftarm/no-collimator'
      ELSEIF(IARM.EQ.2 .AND. COLLIMATOR) THEN
         FILELOC = DAT_DIR(:lnblnk(dat_dir))//
     >'/r-function/rightarm/collimator'
      ELSEIF(IARM.EQ.2 .AND. .NOT.COLLIMATOR) THEN
         FILELOC = DAT_DIR(:lnblnk(dat_dir))//
...
c
      open(11,file=fileloc(:lnblnk(fileloc))//'/line1.dat'
     >,status='old')
      open(12,file=fileloc(:lnblnk(fileloc))//'/line2.dat'
     >,status='old')
      open(13,file=fileloc(:lnblnk(fileloc))//'/line3.dat'
     >,status='old')
      open(14,file=fileloc(:lnblnk(fileloc))//'/line4.dat'
     >,status='old')
      open(15,file=fileloc(:lnblnk(fileloc))//'/line5.dat'
     >,status='old')
      open(16,file=fileloc(:lnblnk(fileloc))//'/line6.dat'
     >,status='old')
      open(17,file=fileloc(:lnblnk(fileloc))//'/line7.dat'
     >,status='old')
      open(18,file=fileloc(:lnblnk(fileloc))//'/line8.dat'
     >,status='old')
      open(19,file=fileloc(:lnblnk(fileloc))//'/line9.dat'
     >,status='old')
      open(20,file=fileloc(:lnblnk(fileloc))//'/line10.dat'
     >,status='old')
      open(21,file=fileloc(:lnblnk(fileloc))//'/line11.dat'
     >,status='old')
      open(22,file=fileloc(:lnblnk(fileloc))//'/line12.dat'
     >,status='old')
      open(23,file=fileloc(:lnblnk(fileloc))//'/dy-dd.dat'
     >,status='old')
      open(30,file=fileloc(:lnblnk(fileloc))//'/min-max.dat'
     >,status='old')
      read(23,*)dy(iarm),dd(iarm)   !y and delta grid spacing
      close(23)
      write(6,*)'initializing r-function arrays'
      open(31,file=fileloc(:lnblnk(fileloc))//'/information.dat'
     >,status='old')
      read(31,'(a24)')daet
